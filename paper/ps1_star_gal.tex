...
is shown in Figure~\ref{fig:ztf_hist}. \todo{re-word after writing the gaia
section} The thresholds appropriate for identifying point-source
counterparts to the ZTF candidates are reported in Table~\ref{tbl:fpr} (note
that these thresholds apply to $\mathtt{nDetections} \ge 3$ sources). Of the
$\sim$1.5$\times 10^{9}$ sources in the ZTF-PS1 catalog, 734,476,355
($\sim$50\%) are classified as stars using the FoM-optimized classification
threshold of 0.83.
...
and proper motion, $\mu$, measurements.\footnote{Any extragalactic sources
will have small low SNR $\varpi$ and $\mu$ measurments.} We define the
...
To determine the threshold for ``high significance'' we select all \aam{XXX}
sources from the \textit{Gaia} database with $\varpi$ and $\mu$ measurments
and $20.5\,\mathrm{mag} \le G \le 21\,\mathrm{mag}$, where $G$ is the mean
brightness measured by \textit{Gaia} in the $G$ filter. These sources, which
are the lowest SNR \textit{Gaia} detections, provide an estimate of the
scatter of the parallax and proper motion significance, as they are too
faint for high-significance detections.\footnote{The typical uncertainty on
parallax for sources this faint is $\sigma_\varpi \approx 2$\,mas
\citep{Lindegren18}. While some of these $\sim$\aam{XXX} sources may be at a
distance $<$500\,pc away, that will not be true for the vast majority, and
thus they will not have significant parallax measurements.} Looking at the
full distribution of parallax and proper motion significance for these faint
sources we find the 99.997-th percentile (roughly 4$\sigma$ for a Gaussian
distribution) corresponds to 8.19 and \aam{YYY}, repectively. Thus, we apply
\textit{conservative cuts} to the parallax significance and the proper
motion significance of $>$8 and $>$\aam{YYY}, respecively, to select
bonafide stars from the \textit{Gaia} data. We further cull this sample by
removing sources with large unit weight error relative to their brightness
(see Appendix~C and Eqn.~C.1 in \citealt{Lindegren18}) and removing sources
outside the range of normal excess flux factors (see
\citealt{Gaia-Collaboration18a,Lindegren18}).
...
tion/chap_cu9val_cu9val/ssec_cu9xma/sssec_cu9xma_extcat.html} has details
about the 810,359,898 \textit{Gaia} sources crossmatched with PS1.} we have
identified \aam{ZZZ} high-confidence stars based on the selection criteria
described above. Of these, \aam{AAA} have counterparts in the ZTF-PS1
catalog (the remaining \aam{ZZZ - AAA} sources have either 0 or $>$1 entires
in the PS1 \textit{StackObjectThin} table). For these high-confidence stars,
we show the distribution of RF classification scores for these stellar
objects in Figure~\ref{fig:gaia}. It is clear from Figure~\ref{fig:gaia}
that the vast majority of stars are classified correctly. \aam{Sentence here
about percentages above different classification thresholds.} We conclude
that the \textit{Gaia} data confirms that our method does an excellent job
of identifying unresolved sources.

\begin{figure}[htb]
 \centering
  \includegraphics[width=3.35in]{./Figures/Gaia_PS1_cat_hist.pdf}
  %
  \caption{ The distribution of RF classification score for sources in the
  ZTF-PS1 star--galaxy catalog that are high-probability stars from
  \textit{Gaia} data. Note that the number counts are shown on a log scale.
  The vertical dotted line shows the FoM-optimized classification threshold,
  sources to the right of the line are classified as stars. The full catalog
  is shown in blue, while Galactic plane sources ($|b| < 10^{\circ}$) are
  shown in orange, and high galactic latitude sources ($|b| > 30^{\circ}$)
  are shown in grey. A very small fraction of these stars have
  $\mathrm{RF\;score} \lesssim 0.5$.}
  %
  \label{fig:gaia}
\end{figure}

Finally, the \aam{ZZZ - AAA} sources that do not have counterparts in the
ZTF-PS1 catalog are assigned an RF classification score of 1 in the ZTF
database. Thus, new transient candidates with positions consistent with
these sources will be flagged as likely stars.
