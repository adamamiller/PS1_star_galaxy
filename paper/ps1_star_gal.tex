...
$\mathtt{nDetections} \ge 3$ sources). Of the $\sim$1.5$\times 10^{9}$ sources in the ZTF-PS1 catalog, 734,476,355 ($\sim$50\%) are classified as stars using the FoM optimized 

Figure~\ref{fig:ztf_hist}


Tuning the star--galaxy
classification thresholds to optimize follow-up of newly discovered
transients is a critical early step in the filtering of ZTF candidates
(e.g., \citealt{Kasliwal:18:ZTF}).
...
The machine learning methodology with the RF algorithm is utilized 
for the construction of the final star--galaxy separation model in this work. 
The RF model is trained using 47,093 PS1 sources with 
{\it HST} COSMOS morphological classifications. 
3,834,627 PS1 sources with SDSS spectral classifications are retained 
as an independent test set for assessing the performance of separation models 
by considering its biases towards to LRGs and faint QSOs. 

For constructing the RF model, 
we introduced ``white flux'' features, corresponding to a weighted mean of the 
relevant features across all five filters $grizy_{\mathrm {PS1}}$ in which a source is detected. 
The training with these features enables the model to provide classifications for every source detected by PS1 
with low reddening bias attributed to local dust columns in the training set. 
Especially, we developed a new relevant star--galaxy separating feature, \texttt{whitePSFKronDist}, 
defined by the orthogonal distance from a line 
($\mathtt{whitePSFFlux} = 0.91375 \times \mathtt{whiteKronFlux}$) 
in the $\mathtt{whitePSFFlux} - \mathtt{whiteKronFlux}$ plane. 
By utilising this, we constructed a straightforward separation model, the simple model, 
as a baseline model for assessing the performance of the RF model. 
Ultimately, we utilized 11 ``white flux'' features for constructing the RF model, 
and the model tuning parameters were optimized via CV on the {\it HST} training set.

CV results for {\it HST} test set with $\mathrm{det}_{i_{\mathrm{PS1}}} = 1$ (consisting of 40,098 sources in total) 
shows that the simple model provides much higher TPR (FoM $= 0.657$) than the PS1 model (FoM $= 0.007$), 
owing to the SNR information in \texttt{whitePSFKronDist} besides the difference of PSF and Kron-aperture measurements. 
The CV performance of the RF model significantly outperforms both of the simple model and the PS1 model 
in terms of the FoM ($=0.707$), the ROC AUC ($=0.973$), and the overall accuracy ($=0.932$). 
Especially for faint sources with $\mathtt{whiteKronMag} > 20$ mag, 
the separation accuracy of the PS1 model drops quickly to $<0.5$, 
while that of the simple model and the RF model exceeds $\sim 0.85$ 
even at $\mathtt{whiteKronMag} = 23$ mag. 

On the other hand, the classification accuracy as evaluated on the SDSS test set 
shows that the SDSS model is superior to both of the simple model and the RF model 
for sources with $\mathtt{whiteKronMag}\gtrsim21$mag. 
We clarified that this reversal of the performance is attributed to the bias in the SDSS test set 
along with the classification methodology of the SDSS model; 
faint sources in the SDSS training set is dominated by QSOs (point sources) and the hard cut on a single value 
of the PSF to \texttt{cModel} tend to classify faint sources as stars. 
We confirmed that the classification accuracy of the RF model outperforms alternative models 
for the sources in the SDSS test set selected to math the ratio of stars to galaxies in the {\it HST} training set, 
and concluded that the RF model provides the most accurate star--galaxy classifications 
among the other models we considered in this work for source distributions that is observed in the real nature. 

The RF model has been deployed in the ZTF real-time pipeline, 
and classifications for PS1 sources with $\mathtt{nDetections} \geq 3$ (1,484,281,394 sources in total) 
is already stored in the ZTF database. 
Alerts on newly discovered ZTF transients including KNe, possibly associated with gravitational waves, 
will be released publicly in near real time. 
}
