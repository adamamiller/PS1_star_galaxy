...
\title{\textbf{something better needed} A PanSTARRS1 Star--Galaxy Separation Model: Application in the ZTF Real-Time Pipeline
\yutaro{Do you have any ideas for the title of this paper?}}
...
accurate classification of astrophysical sources is rapidly increasing.
Specifically, the separation of stars and galaxies in astronomical imaging is
a critical initial step for a wide array of studies, ranging from Galactic
science to large scale structure and cosmology. Here, we present our method
to construct a large, deep catalog of stars and galaxies utilizing
Pan-STARRS1 (PS1) 3$\pi$ survey data, which consists of $\sim$3$\times10^9$
celestial sources with $\lesssim 23.5$ mag. We develop a supervised
machine-learning methodology, using the random forest (RF) algorithm, to
construct the PS1 star--galaxy separation model. We train the model using
$\sim$5$\times10^4$ PS1 sources with \textit{HST} COSMOS morphological
classifications, and assessed its performance with $\sim$4$\times10^6$ PS1
sources with Sloan Digital Sky Survey (SDSS) spectra. We construct 11 ``white
flux'' features, which combine PS1 flux and shape measurements across all 5
PS1 filters, to increase the signal-to-noise ratio relative to any individual
filter and remove potential biases due to the line-of-sight reddenning in the
training set. We compare the performance of the RF model to 3 alternative
models, including the SDSS and PS1 photometric classification models, and
find that the RF model performs best. By number the PS1 catalog is dominated
by faint sources ($m \gtrsim 21$\,mag), and in this regime the RF model
significantly outperforms the SDSS and PS1 models. For time-domain surveys,
star--galaxy separation is critical for identifying the Galactic or
...
\NC{
 In the era of time domain astrophysics implemented by large photometric survey projects, 
 the importance of the automated and accurate classification of astrophysical sources 
 is growing increasingly. 
 Specifically, separating stars and galaxies is the most important steps for many 
 astronomical researches, from Galactic to cosmology. 
 Here, we present a method for constructing a large and deep catalog of stars and galaxies utilizing 
 Pan-STARRS1 (PS1) 3$\pi$ survey imaging catalog, 
 consists $\sim 3\times10^9$ celestial sources with $\lesssim 23.5$ mag. 
 We adopt a supervised machine-learning methodology along with the random forest (RF) algorithm 
 for the construction of the star--galaxy separation model. 
 The RF model is trained using $\sim 5\times10^4$ PS1 sources 
 with {\it HST} COSMOS morphological classifications ({\it HST} training set), 
 and assessed its performance through $\sim 4\times10^6$ PS1 sources with 
 Sloan Digital Sky Survey spectra (SDSS test set). 
 For training the RF model and ultimately mapping between classifications in each source, 
 11 ``white flux'' features are newly developed to classify all PS1 sources without any imputation, 
 the use of a dummy variable, or the concern about the bias of the line-of-sight reddenning in the training set. 
 By the comparison with three simple star--galaxy separation models 
 including that in the SDSS \texttt{photo} pipeline (SDSS model), 
 we confirmed that the RF model provides the most plausible predictions for new sources, 
 by utilizing both of the cross-validated HST training set and the SDSS test set. 
 Especially for faint sources with $\gtrsim 21$ mag, 
 the accuracy is improved $\gtrsim 20$\% from the SDSS model. 
 The RF model have been deployed in the ZTF real-time pipeline to automatically, 
 resulting in the classification of $\sim 1.5\times10^9$ sources, 
 to automatically reject stellar sources from the extragalactic alert stream. 
}
\end{abstract}
...
\begin{deluxetable}{cccc}
    \tablecolumns{4} 
    \tablewidth{0pt} 
    \tablecaption{Classification performance for SDSS Test Set \label{tbl:sdss_per}
    \aam{Should we show uncertainties for them?}}
    \tablehead{ 
    \colhead{model} & \colhead{FoM} & \colhead{Accuracy} & \colhead{ROC AUC}
    }
    \startdata
...
\begin{figure}[t]
 \centering
  ...
  \NC{The vertical dotted-line shows $\mathrm{FPR} = 0.005$, 
  corresponding to the FoM. 
  The inset in the right shows a zoom in around the cross of ROC curves described in the text. }
  The black and the red star shows the FPR and
  TPR if adopting the PS1 model and SDSS \textit{photo} classification cut (rather than
  using the method developed in \citealt{Miller17}), respectively. The RF model delivers
  the highest FoM.}
  %
  \label{fig:roc_sdss}
\end{figure}
...
\begin{table*}[htb]
\begin{center}
\caption{RF model classification threshold and TPR at various fixed FPR values.}
\label{tbl:fpr}
\begin{tabular}{lcc|lccccc}
\hline\hline
                 Source               & $N$ & Accuracy & FPR & 0.005 & 0.01 & 0.02 & 0.05 & 0.1 \\ \hline
...
provide mode accurate classifications than the RF and simple models,
...
\yutaro{Do you need to acknowledge GROWTH} 
