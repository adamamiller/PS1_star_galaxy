...
\multirow{2}{*}{$\mathtt{rKronMag} < 20$}  & \multirow{2}{*}{6,956}& \multirow{2}{*}{$99.0\pm0.1$\%} &
    ...
    \enddata
    \tablecomments{\yutaro{I just noticed that for $r < 20$ and the 0.005 column has 0.995 +/- 0.019, which would go above 1.0. This is not possible, so do you have any idea what's happening here?} 10-fold CV is performed on the entire \textit{HST} training set, but the metrics reported here include only sources that satisfy the selection criteria defined by the first column and $\mathtt{nDetections} \ge 3$.} 
