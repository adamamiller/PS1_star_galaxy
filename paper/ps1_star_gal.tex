...
The AGN sample is not sufficient for defining a cut on $\varpi$ significance because \citet{Lindegren18} require $\varpi$/$\sigma_\varpi < 5$. Instead, we use the 20,568,254 \textit{Gaia} sources with $\varpi$ and $\mu$ measurments, $20.5\,\mathrm{mag} \le G \le 21\,\mathrm{mag}$, where $G$ is the mean
brightness measured by \textit{Gaia} in the $G$ filter, and that pass the cuts defined by Eqn.~(C.1) and (C.2) in \citet{Lindegren18}. These cuts are designed to remove low-confidence parallax measurements in regions of high stellar density. The low SNR detections for these \textit{Gaia} sources provide an estimate of the
