...
was started and had first light at Palomar Observatory in 2017, 
...
ZTF surveys the sky at a rate that is $\sim 15$ times faster than PTF/iPTF. 
...
and/or orbital modulation ({\it e.g.,} \citealt{Kulkarni06, Berger12}). 
The systematic removal of faint stars from extragalactic candidate lists 
enable us to follow-up such exotic sources efficiently under the limitation of time, instrumental, and human resources. 
...
and also to reduce the number of galaxies misclassified as stars (false positives; FPs) 
for identifying fast transients and GW counterparts. 
Despite its much superior performance to that of \texttt{SExtractor} 
which provided the best discriminant between stars and galaxies in PTF images, 
however, they pointed out that the PTF point source-catalog 
...
Our purpose is developing wide and high-performance star--galaxy separation model 
...
and the typical 5$\sigma$ depth of stack image is 23.2 mag in the $r$-band 
with a median seeing of 1.19 arcsec \citep{Tonry12, Schlafly12, Chambers16}. 
...
a description for how the catalog is used by ZTF is provided in Section 6, 
...
through the same criteria with the case of the construction of {\it HST} training set (see \S\ref{sec:hst_train}). 

The accuracy of classifications based on spectra would be higher than that by morphology. 
