...
% using high-significance parallax and proper
% motion measurements it is possible to select a pure sample of unresolved
% stars in the \textit{Gaia} data (e.g., \citealt{Lindegren18}).
...
distribution) corresponds to 8.19 and \aam{YYY}, repectively. Thus, we apply \textit{conservative cuts} to the parallax significance and the proper motion significance of $>$8 and $>$\aam{YYY}, respecively, to select bonafide stars from the \textit{Gaia} data. We further cull this sample by removing sources with large unit weight error relative to their brightness (see Appendix~C and Eqn.\~C.1 in \citealt{Lindegren18}) and removing sources with large excess flux factors (see \citealt{Gaia-Collaboration18a,Lindegren18}).

Using the existing cross match between \textit{Gaia} and PS1 
