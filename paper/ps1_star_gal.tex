...
\begin{figure*}[htb]
 \centering
  ...
  %,bb = 79 3 1061 333
  ]{./Figures/ColorMagDiagramComb.pdf}
  \caption{
...
$i_\mathrm{PS1}$ band), which results in \yutaro{NNN} sources.
...
($\mathrm{TPR} = \mathrm{TP}/(\mathrm{TP} + \mathrm{FP})$, where TP is the
total number of true positive classifications and FP is the number of false
positives) and false positive rate ($\mathrm{FPR} = \mathrm{FP}/(\mathrm{FP}
+ \mathrm{TN})$, where TN is the number of true negatives) vary as a
function of classification threshold. As a reminder, stars are considered
the positive class in this study. For the RF model we vary the
classification threshold from 1 (star) to 0 (galaxy), while for the simple
model we vary the classification threshold from \yutaro{max(wPSFKronDist)}
to \yutaro{max(wPSFKronDist)}, and finally, we create an ROC curve for the
PS1 model by varying the classification threshold from $\mathtt{iPSFMag} -
\mathtt{iKronMag} > \yutaro{max(iPSF-iKron)}\;\mathrm{mag}$ to
$\yutaro{min(iPSF-iKron)}\;\mathrm{mag}$.

Summary statistics showing the superior performance of the RF model relative
to the simple and PS1 models are presented in Table~\ref{}. [\yutaro{We
should maybe include a table with the FoM+threshold, accuracy, ROC AUC for
each of these models. What do you think?}] These statistics include the FoM,
the integrated area under the ROC curve (ROC AUC), and the overall accuracy
of the 3 models as evaluted on the subset of \textit{HST} training set
sources with $i_\mathrm{PS1}$ detections. We use 10-fold CV to measure the
summary statistics, with identical folds for each model. Strictly speaking,
this CV procedure is only needed for the RF model, which needs to be
re-trained for every fold, but testing the simple and PS1 models on the
individual folds provides an estimate in the scatter of the final reported
metrics. From Table~\ref{} it is clear that the RF model greatly outperforms
...
\NC{Figure \ref{fig:cvroc_hst} shows the CV ROC curve of each models, 
...
%  ,bb = 0 0 900 360
  ]{./Figures/Accuracy_SDSS_ML_model.pdf}
  \caption{
