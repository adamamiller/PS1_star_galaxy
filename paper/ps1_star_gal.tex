...
\yutaro{max(wPSFKronDist)}$ to \yutaro{max(wPSFKronDist)} and
$\mathtt{iPSFMag} - \mathtt{iKronMag} >
\yutaro{max(iPSF-iKron)}\;\mathrm{mag}$ to
$\yutaro{min(iPSF-iKron)}\;\mathrm{mag}$, respecitively.
...
\begin{figure}[t]
 \centering
  \includegraphics[width=3.5in]{./Figures/CV_Accuracy_HST.pdf}
  %
...
(\yutaro{NNN} total) in Figure~\ref{fig:roc_sdss}. The FoM for the PS1,
