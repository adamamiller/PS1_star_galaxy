...
\begin{figure*}[t]
 \centering
  \includegraphics[width=6in]{./Figures/SDSS_acc_mag.pdf}
  %
...
regime, the SDSS test set is dominated by stars, as shown by the PDFs in the
same panel of Figure~\ref{fig:roc_sdss}. This is counter to what is observed
in nature (at high galactic latitudes), as galaxy number counts exceed those
of stars starting for $r \gtrsim 20\,\mathrm{mag}$, with the number of
galaxies (e.g., \citealt{Yasuda01}). Thus, the SDSS test set is biased,
because the faintest sources they have observed are distant QSOs. \todo{fix this last sentence}
