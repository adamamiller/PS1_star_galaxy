...
\texttt{whiteFPSFFKronDist} and \texttt{whiteFPSFFApDist} distribution of
resolved and unresolved sources is shown in Figures~\ref{fig:psf_kron} and
\ref{fig:psf_ap}, respectively.
...
We show the CV accuracy of the RF, simple, and PS1 models\footnote{The PS1
model is defined by a single hard cut on the PSF--Kron flux ratio measured in
the $i_\mathrm{PS1}$ band \citep[for further details see][]{Tachibana18}.} in
Figure~\ref{fig:accuracy_mag}. As in \citet{Tachibana18}, we find that the RF
...
\begin{figure}
    ...
    %
    \caption{Model accuracy in 0.5\,mag bins for each of the RF, simple, and
    PS1 models as a function of \texttt{whiteKronMag}. Filled, but
    transparent, Gaussian KDEs along the bottom axis show the relative
    distribution of resolved and unresolved sources in the HST training set.
    Unresolved sources dominate at $\mathtt{whiteKronMag} < 18$\,mag, while
    resolved sources dominate at $\mathtt{whiteKronMag} > 20$\,mag. The RF
    model provides the highest accuracy, especially for faint sources
    ($\mathtt{whiteKronMag} \gtrsim 21.5$\,mag).}
    %
    \label{fig:accuracy_mag}
\end{figure}    

As expected, the accuracy of each model shown in Figure~\ref{fig:accuracy_mag}
decreases with the S/N. Interestingly, the accuracy curve for the RF model is
nearly flat for \texttt{whiteKronMag} between $\sim$22 and 24\,mag.
